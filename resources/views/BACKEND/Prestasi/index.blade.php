                        ...
                        <tbody>
                            @forelse ($data as $index => $row)
                                <tr>
                                    ...
                                    <td>
                                        @if ($row->foto)
                                            <img src="{{ asset('uploads/prestasi/' . $row->foto) }}" alt="Foto prestasi" style="width: 64px; height: 64px; object-fit: cover; border-radius: 6px;">
                                        @else
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('prestasi.destroy', $row->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus data prestasi ini?')">
                                            @csrf
                                            @method('DELETE')
